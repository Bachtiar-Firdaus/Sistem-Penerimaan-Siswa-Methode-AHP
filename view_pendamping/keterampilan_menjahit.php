...
<div class="containeredit" style="overflow: scroll;">
  ...
  <h2>Keterampilan Menjahit</h2> <hr>

  <form method="post" action="view_pendamping/sortir_menjahit.php">
...
// $result = mysqli_query($mysqli, "SELECT * FROM tbl_kegiatan where jurusann ='menjahit'"); 

$result = mysqli_query($mysqli, "SELECT DISTINCT nama_siswa FROM tbl_kegiatan where jurusann ='menjahit'  order by id_kegiatan ASC; ");
...
<select name="nama_pendamping">
  ...
  include_once("\insert\config.php");
// $result = mysqli_query($mysqli, "SELECT * FROM tbl_kegiatan");

$result = mysqli_query($mysqli, "SELECT DISTINCT nama_pendamping FROM tbl_kegiatan order by id_kegiatan ASC; ");

// SELECT DISTINCT nama_siswa FROM tbl_kegiatan order by id_siswa ASC; 
...
ON tbl_cs2.id=tbl_kegiatan.id_kegiatan  where jurusann = 'Menjahit'");
    ...
    while($user_data = mysqli_fetch_array($result)) {         
        ...
        echo "<td>|| <a href='/siks_ahp/view_pendamping/edit_menjahit.php?id_kegiatan=$user_data[id_kegiatan]'>Edit</a> || <a href='/siks_ahp/view_pendamping/delete_menjahit.php?id_kegiatan=$user_data[id_kegiatan]'>Delete</a></td></tr>";      
    }
    ?>
